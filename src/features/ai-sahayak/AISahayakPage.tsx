...
export default function AISahayakPage() {
  ...
  const ask=async(text=msg)=>{if(!text.trim()||busy)return;if(!token){setAnswer(t("page.ai.loginRequired"));return;}setBusy(true);setAnswer("");try{const r=await aiApi.ask(token,text.trim());setAnswer(r.answer);setMsg("");}catch(e){setAnswer(e instanceof Error?e.message:t("page.ai.error"));}finally{setBusy(false)}};
  const renderAnswer=(value:string)=>value.split(/\n{2,}/).map((part,i)=>{const lines=part.split(/\n/);const bullet=lines.every(line=>/^\s*[-*•]\s+/.test(line));return bullet?<ul key={i}>{lines.map((line,j)=><li key={j}>{line.replace(/^\s*[-*•]\s+/,"")}</li>)}</ul>:<p key={i}>{lines.map((line,j)=><span key={j}>{line}{j<lines.length-1&&<br/>}</span>)}</p>});
  ...
  return <main className="gls-app-page gls-modern-page">
    ...
    <section className="gls-shell gls-page-body"><div className="gls-reference-heading"><div><span>{t("page.ai.suggested")}</span><h2>{t("page.ai.title")}</h2><p>{t("page.ai.desc")}</p></div><div className="gls-live-chip">● {t("page.ai.online")}</div></div>
      <div className="gls-ai-reference-layout"><div className="gls-chat-card"><div className="gls-chat-head"><div className="gls-avatar"><Bot size={21}/></div><div><b>{t("page.ai.title")}</b><small>● {t("page.ai.online")}</small></div></div><div className="gls-chat-body"><div className={`gls-bubble gls-ai-answer ${busy ? "is-loading" : ""}`}>{busy?<><span className="gls-spinner gls-spinner-dark"/> {t("auth.pleaseWait")}</>:answer?renderAnswer(answer):<p>{t("page.ai.desc")}</p>}</div><div className="gls-section-label"><Sparkles size={15}/>{t("page.ai.suggested")}</div><div className="gls-question-grid">{questions.map(k=><button type="button" key={k} onClick={()=>quick(k)}>{t(`page.ai.${k}`)}<span>→</span></button>)}</div></div><div className="gls-chat-input"><input value={msg} onChange={e=>setMsg(e.target.value)} placeholder={t("page.ai.search")} onKeyDown={e=>{if(e.key==="Enter")void ask()}}/><button type="button" onClick={()=>void ask()} disabled={busy||!msg.trim()} aria-label={t("page.ai.search")}>{busy?<><span className="gls-spinner" aria-label="loading"/><span className="sr-only">{t("auth.pleaseWait")}</span></>:<Send size={18}/>}</button></div></div>
